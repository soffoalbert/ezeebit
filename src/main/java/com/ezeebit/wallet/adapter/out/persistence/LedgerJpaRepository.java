...
interface LedgerJpaRepository extends JpaRepository<LedgerEntryEntity, Long> {
    ...
    @Query("select e from LedgerEntryEntity e where e.accountId = :accountId order by e.id desc")
    List<LedgerEntryEntity> findByAccount(@Param("accountId") long accountId, Pageable pageable);
